...
from utils import check_ok_gesture, check_thumb_up, calculate_center
...
def process_frame(img):
    ...
    if results.multi_hand_landmarks:  # 如果有检测到手
        ...
        for hand_idx in range(len(results.multi_hand_landmarks)):
            ...
            if check_ok_gesture(hand) :
                ok_x = int((hand.landmark[4].x + hand.landmark[8].x) / 2 * w)
                ok_y = int((hand.landmark[4].y + hand.landmark[8].y) / 2 * h)
                img = cv2.putText(img, "OK", (ok_x, ok_y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            elif check_thumb_up(hand) :
                thumb_x = int(hand.landmark[4].x * w)
                thumb_y = int(hand.landmark[4].y * h)
                img = cv2.putText(img, "Thumb Up", (thumb_x, thumb_y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
